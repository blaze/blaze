...
def test_path_split():
    expr = t.amount.sum() + 1
    assert path_split(t, expr) is t.amount.sum()

    expr = t.amount.distinct().sort()
    assert path_split(t, expr) is t.amount.distinct()

    t2 = transform(t, id=t.id * 2)
    expr = by(t2.id, amount=t2.amount.sum()).amount + 1
    assert path_split(t, expr) is by(t2.id, amount=t2.amount.sum())

    expr = count(t.amount.distinct())
    assert path_split(t, expr) is t.amount.distinct()

    expr = summary(total=t.amount.sum())
    assert path_split(t, expr) is expr
...
def test_sum():
    ...
    assert chunk_expr is chunk.amount.sum(keepdims=True)

    assert isscalar(agg.dshape.measure)
    assert agg_expr is sum(agg)
...
def test_mean():
    ...
    assert agg_expr is agg.total.sum() / agg['count'].sum()
...
def test_sum_with_axis_argument():
    ...
    assert chunk_expr is chunk.x.sum(axis=0, keepdims=True)
    assert agg_expr is agg.sum(axis=0)
...
def test_sum_with_keepdims():
    ...
    assert isscalar(agg.dshape.measure)
    assert agg_expr is sum(agg, keepdims=True)
...
def test_distinct():
    ...
    assert chunk.schema == t.schema
    assert chunk_expr is chunk.amount.distinct()

    assert isscalar(agg.dshape.measure)
    assert agg_expr is count(agg.distinct())
...
def test_summary():
    ...
    assert chunk.schema == t.schema
    assert chunk_expr is summary(
        a=chunk.amount.count(),
        b=chunk.id.sum(),
        keepdims=True,
    )

    # assert not agg.schema == dshape('{a: int32, b: int32}')
    assert agg_expr is summary(
        a=agg.a.sum(),
        b=agg.b.sum() + 1,
    )
...
def test_by_sum():
    ...
    assert chunk.schema == t.schema
    assert chunk_expr is by(chunk.name, total=chunk.amount.sum())

    assert not isscalar(agg.dshape.measure)
    assert agg_expr is by(agg.name, total=agg.total.sum())
...
def test_by_count():
    ...
    assert chunk_expr is by(chunk.name, total=chunk.amount.count())
...
def test_embarassing_rowwise():
    ...
    assert chunk_expr is chunk.amount + 1
    assert agg_expr is agg


def test_embarassing_selection():
    (chunk, chunk_expr), (agg, agg_expr) = split(t, t[t.amount > 0])

    assert chunk_expr is chunk[chunk.amount > 0]
    assert agg_expr is agg
...
def test_embarassing_like():
    ...
    assert chunk_expr is chunk[chunk.name.like('Alice*')]
    assert agg_expr is agg
...
def test_nd_chunk():
    ...
    assert chunk_expr is chunk.sum(keepdims=True)

    assert agg.shape == (6, 4)
    assert agg_expr is agg.sum()
...
def test_nd_chunk_axis_args():
    ...
    assert chunk_expr is chunk.sum(keepdims=True, axis=0)
...
def test_reductions():
    ...
    assert isscalar(agg.dshape.measure)
    assert agg_expr is agg.distinct().count()
    ...
    assert isscalar(agg.dshape.measure)
    assert agg_expr is agg.distinct().count(keepdims=True)
...
def test_by_with_single_field_child():
    ...
    assert chunk_expr is by(chunk, total=chunk.sum())
...
def test_splittable_apply():
    ...
    assert chunk_expr is chunk.amount.apply(f, 'var * int', splittable=True)
...
def test_elemwise_with_multiple_paths():
    ...
    assert chunk_expr is summary(x=chunk.x.sum(), y=chunk.y.sum())
    assert agg_expr is agg.x / agg.y
