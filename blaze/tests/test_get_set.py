...
class getitem(unittest.TestCase):
    ...
    def test_scalar(self):
        ...
        self.assertEqual(ddesc_as_py(a[0]._data), 0)

    def test_1d(self):
        a = blaze.array(np.arange(3), caps=self.caps)
        # print("a:", a, self.caps)
        self.assertEqual(ddesc_as_py(a[0:2]._data), [0,1])

    def test_2d(self):
        a = blaze.array(np.arange(3*3).reshape(3,3), caps=self.caps)
        self.assertEqual(ddesc_as_py(a[1]._data), [3,4,5])
...
class setitem(unittest.TestCase):
    ...
    def test_scalar(self):
        a = blaze.array(np.arange(3), caps=self.caps)
        a[0] = 1
        self.assertEqual(ddesc_as_py(a[0]._data), 1)

    def test_1d(self):
        a = blaze.array(np.arange(3), caps=self.caps)
        a[0:2] = 2
        self.assertEqual(ddesc_as_py(a[0:2]._data), [2,2])

    def test_2d(self):
        a = blaze.array(np.arange(3*3).reshape(3,3), caps=self.caps)
        a[1] = 2
        self.assertEqual(ddesc_as_py(a[1]._data), [2,2,2])
