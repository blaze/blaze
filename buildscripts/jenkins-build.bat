...
call C:\Anaconda\Scripts\conda create --yes --channel https://conda.binstar.org/mwiebe -p %PYENV_PREFIX% python=%PYTHON_VERSION%  cython=0.19 scipy ply dynd-python nose flask pyparsing pyyaml setuptools dateutil pip pytables sqlalchemy h5py multipledispatch pandas || exit /b 1
echo on
...
call nosetests --with-doctest --with-xunit --xunit-file=../test_results.xml blaze/expr blaze/compute blaze/data || exit /b 1
